...
def gradient_check(theta1,theta2,X,y,e):
    ...
    numeric_grad=np.array(numeric_grad) #近似梯度矩阵
    reg_D1,reg_D2=regularized_gradient(theta1,theta2,X,y,lmd=1)
...
def nn_training(X,y):
    ...
    res=opt.minimize(fun=cost_reg,x0=init_theta,args=(X,y,1),method='TNC',jac=regularized_gradient,options={'matrix':400})
...
def accuracy(theta,X,y):
    ...
    print(classification_report(y,y_pred)) # 7和10的精确度很低，平均低了9个百分点
